...
export default function ContactFormClassic() {
  ...
  // ── Success — horizontal banner with left accent border ──
  if (state === "success") {
    return (
      <div className="flex items-start gap-5 p-6" style={{ borderLeft: `4px solid ${c.accent}`, backgroundColor: c.accentLight }}>
        <div className="w-10 h-10 shrink-0 flex items-center justify-center font-bold text-white text-sm" style={{ backgroundColor: c.accent, borderRadius: c.radius }}>✓</div>
        <div className="flex-1">
          <p className="font-semibold mb-1" style={{ color: c.heading }}>Message sent successfully</p>
          <p className="text-sm" style={{ color: c.muted }}>We&apos;ll get back to you within one business day.</p>
        </div>
        <button onClick={() => setState("idle")} className="text-xs shrink-0 mt-1" style={{ color: c.muted }}>✕ Close</button>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 overflow-hidden" style={{ border: `1px solid ${c.border}`, borderRadius: c.radius }}>
      {/* Left — info panel */}
      <div className="lg:col-span-2 flex flex-col justify-between p-8 gap-8" style={{ backgroundColor: c.accent }}>
        <div>
        ...
        <div className="flex flex-col gap-5">
          {companyInfo.phone && (
            <a href={`tel:${companyInfo.phone}`} className="flex items-start gap-3">
      ...
      <form onSubmit={handleSubmit} className="lg:col-span-3 flex flex-col gap-5 p-8" noValidate>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" required value={fields.name} onChange={set("name")} placeholder="Oliver James Smith" className="form-input" style={r} />
          </div>
          <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" value={fields.company} onChange={set("company")} placeholder="Walker & Co Ltd" className="form-input" style={r} />
          </div>
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="form-label">Enquiry type</label>
          <select value={fields.enquiryType} onChange={set("enquiryType")} className="form-input" style={r}>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea required rows={5} value={fields.message} onChange={set("message")} placeholder="Tell us how we can help…" className="form-input resize-none" style={r} />
        </div>
        ...
        <button type="submit" disabled={state === "loading"} className="form-btn mt-2" style={r}>
