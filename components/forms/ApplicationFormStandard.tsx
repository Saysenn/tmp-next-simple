// Batch 1 — ApplicationFormStandard (single-column layout, all fields optional)
"use client";
...
export default function ApplicationFormStandard() {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="afs-name" className="form-label">Full name <span className="text-red-500">*</span></label>
          <input id="afs-name" type="text" required value={fields.name} onChange={set("name")} placeholder="Jane Smith" className="form-input" />
        </div>
        <div className="flex flex-col gap-1.5">
          <label htmlFor="afs-email" className="form-label">Email address <span className="text-red-500">*</span></label>
          <input id="afs-email" type="email" required value={fields.email} onChange={set("email")} placeholder="jane@company.com" className="form-input" />
        </div>
      </div>
      ...
      {(showPhone || showPosition) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {showPhone && (
            <div className="flex flex-col gap-1.5">
              <label htmlFor="afs-phone" className="form-label">
                Phone <span className="font-normal" style={{ color: "var(--section-muted, var(--text-muted))" }}>(optional)</span>
              </label>
              <input id="afs-phone" type="tel" value={fields.phone} onChange={set("phone")} placeholder="+44 7700 900000" className="form-input" />
            </div>
          )}
          {showPosition && (
            <div className="flex flex-col gap-1.5">
              <label htmlFor="afs-position" className="form-label">
                Position <span className="font-normal" style={{ color: "var(--section-muted, var(--text-muted))" }}>(optional)</span>
              </label>
              <input id="afs-position" type="text" value={fields.position} onChange={set("position")} placeholder="e.g. Senior Designer" className="form-input" />
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {!cvFile ? (
          ...
          >
            ...
            <input id="afs-cv" ref={fileInputRef} type="file" accept={acceptAttr} onChange={handleFileChange} className="hidden" />
          </label>
        ) : (
          <div className="form-file-pill">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs shrink-0" style={{ color: "var(--section-muted, var(--text-muted))" }}>{(cvFile.size / 1024 / 1024).toFixed(1)}MB</span>
            </div>
            <button type="button" onClick={removeFile} className="shrink-0 text-xs transition hover:text-red-500" style={{ color: "var(--section-muted, var(--text-muted))" }}>✕ Remove</button>
          </div>
        )}
      ...
      {showCoverLetter && (
        <div className="flex flex-col gap-1.5">
          <label htmlFor="afs-cover" className="form-label">
            Cover letter <span className="font-normal" style={{ color: "var(--section-muted, var(--text-muted))" }}>(optional)</span>
          </label>
          <textarea id="afs-cover" rows={5} value={fields.coverLetter} onChange={set("coverLetter")} placeholder="Tell us why you'd be a great fit…" className="form-input resize-none" />
        </div>
      )}
