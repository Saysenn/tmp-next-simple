  ...
  return (
    <section style={{ background: c.bg, overflow: "hidden", position: "relative", minHeight: 480 }}>
      <style>{`
        ...
        @keyframes marquee-up-slow  { from { transform: translateY(0); } to { transform: translateY(-50%); } }
        .rm-col1 { animation: marquee-up      28s linear infinite; }
        .rm-col2 { animation: marquee-up-slow 38s linear infinite; }
      `}</style>
      ...
      <div className="section-inner py-20 lg:py-28 relative z-10">
        <div style={{ maxWidth: 380, width: "100%" }} data-animate="fade-up">
          ...
          <h2 style={{ fontSize: "clamp(1.8rem, 3.5vw, 3rem)", color: c.heading, fontWeight: 700, lineHeight: 1.15 }}>
            Trusted by Employers and Candidates Across the UK
          </h2>
        </div>
      </div>

      {/* Diagonal marquee — right side */}
      <div style={{ position: "absolute", top: "-20%", right: "-2%", width: "60%", height: "140%", overflow: "hidden", pointerEvents: "none" }}>
        ...
        <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: 80, background: `linear-gradient(to right, ${c.bg}, transparent)`, zIndex: 5 }} />

        <div style={{ display: "flex", gap: 14, transform: "rotate(-16deg) translateX(-8%)", transformOrigin: "top center", height: "100%" }}>
          {/* Column 1 */}
          <div style={{ flex: "0 0 260px" }}>
            <div className="rm-col1" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[...reviewsData, ...reviewsData].map((r, i) => <Card key={i} r={r} />)}
            </div>
          </div>
          {/* Column 2 — offset + slower */}
          <div style={{ flex: "0 0 260px", marginTop: 80 }}>
            <div className="rm-col2" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[...reviewsData, ...reviewsData].map((r, i) => <Card key={i} r={r} />)}
            </div>
          </div>
        </div>
