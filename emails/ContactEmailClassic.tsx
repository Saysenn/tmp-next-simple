      ...
      <Body style={s.body}>
        <Container style={s.container}>
          ...
          <Section style={s.content}>
            ...
            <Text style={s.senderEmail}>
              <Link href={`mailto:${email}`} style={{ color: ACCENT, textDecoration: "none" }}>{email}</Link>
              {phone && (
                <> · <Link href={`tel:${phone}`} style={{ color: "#6b7280", textDecoration: "none" }}>{phone}</Link></>
              )}
            </Text>
...
const s = {
  ...
  senderName:  { margin: "0 0 4px", color: "#111827", fontSize: "17px", fontWeight: "600" },
  senderEmail: { margin: "0 0 24px", color: "#374151", fontSize: "14px" },
